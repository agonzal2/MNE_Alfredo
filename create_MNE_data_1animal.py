...
channel_names=['hpc_mid_deep', 'hpc_mid_sup', 'hpc_ros_deep', 'hpc_ros_sup', 'pfc_deep', 
               ...
               'EMG1', 'EMG2', 'cb_deep', 'cb_sup', 'hp_caudal_deep', 'hpc_caudal_sup','Opto']
channel_types=['eeg','eeg','eeg','eeg','eeg','eeg','eeg','eeg','eeg','eeg','emg','emg', 'eeg','eeg',
               'eeg','eeg', 'stim']
...
#####This is to filter.
#filt=custom_raw.filter(8, 16, fir_design='firwin')
...
#epochs.plot(picks=[30,48], n_epochs=1, block=True, scalings= 'auto') #block episodes. 
#

###This below can make a plot of all epochs together. 

#epochs.plot_image(picks=[30,48], combine='mean', group_by='type', sigma=2., cmap="rainbow")
...
freqs = np.arange(3., 20., 1.)
vmin, vmax = 90, 160.  # Define our color limits.
...
#fmin, fmax = freqs[[0, -1]]
#
#for width, ax in zip((1, 2, 3.0), axs):
...
for n_cycles, ax in zip(all_n_cycles, axs):
    ...
    power.plot( vmin=vmin, vmax=vmax,
               axes=ax, dB=True, show=False, colorbar=True)#Key was DB as poer of frequencies is very tight.
